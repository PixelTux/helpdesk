<template>
    <div class="sticky bottom-0 z-10 shrink-0 border-t bg-card p-4">
        ...
        <div v-if="!isFormExpanded" class="flex items-center gap-2">
            <div class="flex-1">
                <input
                    ...
                    class="w-full rounded-md border-2 px-3 py-2 text-sm focus:outline-none"
            ...
            <div class="flex items-center gap-1">
                <button
                    :class="`flex items-center gap-1 rounded-md px-2 py-1 text-xs transition-all ${
                        activeTab === 'reply' ? 'bg-primary text-primary-foreground shadow-sm' : 'bg-muted text-muted-foreground hover:bg-muted/50'
                    }`"
                ...
                <button
                    @click="toggleFormExpanded"
                    class="flex items-center rounded-md bg-primary px-3 py-1 text-xs text-primary-foreground disabled:opacity-50"
                >
                    Send
                </button>
            </div>
        ...
        <div v-if="isFormExpanded">
            ...
            <div class="mb-4 flex gap-1 rounded-lg bg-muted/50 p-1">
                <button
                    :class="`flex flex-1 items-center gap-2 rounded-md px-3 py-2 text-sm transition-all ${
                        activeTab === 'reply' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted'
                    }`"
                    @click="setActiveTab('reply')"
                >
                ...
                <button
                    :class="`flex flex-1 items-center gap-2 rounded-md px-3 py-2 text-sm transition-all ${
                        activeTab === 'internal' ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted'
                    }`"
                    @click="setActiveTab('internal')"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div v-if="activeTab === 'reply'" class="space-y-3">
                <TiptapEditor ref="replyEditor" v-model="replyContent" placeholder="Type your reply to the contact..." />
                <div class="flex justify-end">
                    <button
                        @click="handleSendReply"
                        :disabled="isReplyEmpty"
                        class="flex items-center gap-1 rounded-md bg-primary px-3 py-1 text-sm text-primary-foreground disabled:opacity-50"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div v-if="activeTab === 'internal'" class="space-y-3">
                <TiptapEditor ref="internalEditor" v-model="internalNote" placeholder="Add an internal note (only visible to your team)..." />
                <div class="flex justify-end">
                    <button
                        @click="handleSendInternalNote"
                        :disabled="isInternalNoteEmpty"
                        class="flex items-center gap-1 rounded-md bg-secondary px-3 py-1 text-sm text-secondary-foreground disabled:opacity-50"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path
                                stroke-linecap="round"
...
function handleSendReply() {
    ...
    router.post(
        route('helpdesk.messages.store', { conversation: props.conversation.id }),
        {
...
function handleSendInternalNote() {
    ...
    router.post(
        route('conversations.internal-note', { conversation: props.conversation.id }),
        {
