    ...
    <div class="space-y-3">
        ...
        <div class="relative">
            ...
            <div
                ...
                class="absolute z-10 mt-1 max-h-60 w-full overflow-y-auto rounded-md border border-sidebar-border bg-primary-foreground shadow-lg"
            >
                <!-- Existing tags -->
                <button
                    ...
                    @click="selectTag(tag)"
                    :class="[
                        'flex w-full items-center justify-between px-4 py-2 text-left hover:bg-muted-foreground/35',
                ...
                <button
                    ...
                    @click="createNewTag"
                    :class="[
                        'flex w-full items-center px-4 py-2 text-left text-green-600 hover:bg-muted-foreground/35',
...
const filteredTags = computed(() => {
    ...
    return props.availableTags.filter((tag) => tag.name.toLowerCase().includes(query) && !selectedIds.includes(tag.id)).slice(0, 10); // Limit to 10 suggestions
});

const canCreateNew = computed(() => {
    if (!newTagName.value || newTagName.value.length < 2) return false;

    const exactMatch = props.availableTags.some((tag) => tag.name.toLowerCase() === newTagName.value.toLowerCase());

    const alreadySelected = selectedTags.value.some((tag) => tag.name.toLowerCase() === newTagName.value.toLowerCase());
