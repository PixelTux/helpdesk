    ...
    <div class="mx-auto max-w-6xl space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-muted-foreground">Manage knowledge base tags and their usage.</p>
            </div>
            <div class="space-x-2">
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <div class="flex flex-col gap-4 sm:flex-row">
                <div class="flex-1">
                    <Input v-model="filters.search" placeholder="Search tags..." @input="handleSearch" class="max-w-sm" />
                </div>
        ...
        <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="border-b bg-gray-50">
                        <tr>
                            <th class="cursor-pointer p-4 text-left font-medium text-gray-900 hover:bg-gray-100" @click="sort('name')">
                                <div class="flex items-center gap-2">
                                    Name
                                    <ArrowUpDown class="h-4 w-4 text-gray-400" />
                                </div>
                            </th>
                            <th class="p-4 text-left font-medium text-gray-900">Slug</th>
                            <th
                                class="cursor-pointer p-4 text-left font-medium text-gray-900 hover:bg-gray-100"
                            ...
                            <th class="cursor-pointer p-4 text-left font-medium text-gray-900 hover:bg-gray-100" @click="sort('created_at')">
                                <div class="flex items-center gap-2">
                                    Created
                                    <ArrowUpDown class="h-4 w-4 text-gray-400" />
                                </div>
                            </th>
                            <th class="p-4 text-right font-medium text-gray-900">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        <tr v-for="tag in tags.data" :key="tag.id" class="hover:bg-gray-50">
                            ...
                            <td class="space-x-2 p-4 text-right">
                                <Button variant="outline" size="sm" @click="editTag(tag)">
                                    <Edit class="h-4 w-4" />
                                </Button>
                                <Button
                                    variant="outline"
            ...
            <!-- Empty State -->
            <div v-if="tags.data.length === 0" class="py-12 text-center">
                <Tag class="mx-auto mb-4 h-16 w-16 text-gray-400" />
                <h3 class="mb-2 text-lg font-medium text-gray-900">No tags found</h3>
                <p class="mb-4 text-gray-500">
                    {{ filters.search ? 'No tags match your search criteria.' : 'Get started by creating your first tag.' }}
                </p>
                <Button v-if="!filters.search" @click="showCreateModal = true"> Create Tag </Button>
            </div>
        </div>

        <!-- Pagination -->
        <div v-if="tags.data.length > 0" class="flex items-center justify-between">
            <p class="text-sm text-gray-700">Showing {{ tags.from }} to {{ tags.to }} of {{ tags.total }} results</p>
        ...
        <Dialog v-model:open="showDeleteDialog">
            <DialogContent class="sm:max-w-md">
                ...
                <DialogFooter>
                    ...
                    <Button variant="destructive" @click="deleteTag" :disabled="deleteForm.processing">
...
import { ArrowUpDown, Edit, Plus, Tag, Trash2 } from 'lucide-vue-next';
