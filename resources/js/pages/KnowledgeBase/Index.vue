    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 px-4 py-6">
            ...
            <div class="space-y-6">
                ...
                <!-- AI Assistant -->
                <div class="rounded-lg border border-muted bg-gradient-to-r from-chart-4 to-chart-1 p-6 shadow-2xl">
                    <AIAnswerGenerator />
                </div>

                <!-- Search Bar -->
                <div class="max-w-md">
                    <Input v-model="searchQuery" placeholder="Search articles..." @input="handleSearch" class="w-full" />
                </div>
            </div>

            <!-- Active Filters -->
            <div class="flex flex-wrap gap-2" v-if="searchQuery || selectedTag">
                <div v-if="searchQuery" class="flex items-center gap-2 rounded-full bg-blue-100 px-3 py-1 text-sm text-blue-800">
                    <span>Search: "{{ searchQuery }}"</span>
                    <button @click="clearSearch" class="hover:text-blue-600">&times;</button>
                </div>
                <div v-if="currentTag" class="flex items-center gap-2 rounded-full bg-green-100 px-3 py-1 text-sm text-green-800">
            ...
            <div v-if="articles.data.length > 0" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                <Card v-for="article in articles.data" :key="article.id" class="transition-shadow hover:shadow-lg">
                    <CardHeader>
                        <CardTitle>
                            <div v-if="article.highlighted_title" v-html="article.highlighted_title" class="search-highlight"></div>
                            <div v-else>{{ article.title }}</div>
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div v-if="article.highlighted_excerpt" v-html="article.highlighted_excerpt" class="search-highlight text-gray-600"></div>
                        ...
                        <div v-if="article.tag_names.length > -1" class="mt-3 flex flex-wrap gap-1">
                            <span v-for="tagName in article.tag_names" :key="tagName" class="rounded bg-input px-2 py-1 text-xs">
                                {{ tagName }}
                            </span>
                        </div>
                    </CardContent>
                    <CardFooter>
                        <Link :href="route('knowledge-base.show', article.slug)" class="font-medium text-blue-600 hover:text-blue-800">
                            Read more →
                        </Link>
                    </CardFooter>
                </Card>
                ...
                <nav class="flex items-center space-x-1">
                    <template v-for="link in articles.links" :key="link.url || link.label">
                        <Link
                            v-if="link.url"
                            :href="link.url"
                            :class="['px-3 py-2 text-sm', link.active ? 'rounded bg-blue-600 text-white' : 'rounded text-blue-600 hover:bg-blue-50']"
                            v-html="link.label"
                        />
                        <span
                            v-else
                            :class="['cursor-not-allowed px-3 py-2 text-sm text-gray-400', link.active ? 'rounded bg-blue-600 text-white' : '']"
                            v-html="link.label"
                        />
                    </template>
                </nav>
